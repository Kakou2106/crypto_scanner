...
class QuantumScraper:
    ...
    async def scrape_icodrops(self) -> List[Project]:
        ...
        html = await self._get(url)
        if not html:
            logger.warning("ICOdrops inaccessible, no projects found")
            return projects
        soup = BeautifulSoup(html, "html.parser")
    ...
    async def scrape_airdropsio(self) -> List[Project]:
        ...
        html = await self._get(url)
        if not html:
            logger.warning("Airdrops.io inaccessible, no projects found")
            return projects
        soup = BeautifulSoup(html, "html.parser")
    ...
    async def scrape_binance_launchpool(self) -> List[Project]:
        ...
        html = await self._get(url)
        if not html:
            logger.warning("Binance Launchpool inaccessible, no projects found")
            return projects
        soup = BeautifulSoup(html, "html.parser")
        cards = soup.select('div.css-1m9uhkq')
        for card in cards:
            try:
                ...
                continue
        logger.info(f"Binance Launchpool: {len(projects)} projets found")
        return projects
    ...
    async def scrape_launchpadio(self) -> List[Project]:
        ...
        html = await self._get(url)
        if not html:
            logger.warning("Launchpad.io inaccessible, no projects found")
            return projects
        soup = BeautifulSoup(html, "html.parser")
        ...
        for item in items:
            try:
                ...
                continue
        logger.info(f"Launchpad.io: {len(projects)} projets found")
        return projects
...
class AdvancedAnalyzer:
    ...
    async def run(self, projects: List[Project], db: DBManager):
        count=0
        for p in projects:
...
async def main():
    ...
    await analyzer.run(projects, db)
