# QUANTUM_SCANNER_ULTIME_210K_FIX.py
...
class QuantumScannerUltime:
    def __init__(self):
        ...
        self.MAX_MC = 210000  # 🚨 SEUL CHANGEMENT: 100000 → 210000 🚨
        self.init_db()
    ...
    async def analyser_projet_complet(self, projet):
        ...
        go_decision = (
            projet['mc'] <= self.MAX_MC and  # 🚨 ICI ÇA UTILISE 210K MAINTENANT
            score >= 70 and
...
"""
    ...
    async def run_scan_once(self):
        """🚨 NOUVELLE FONCTION: Scan unique pour GitHub Actions"""
        print("🚀 LANCEMENT SCAN QUANTUM 210K UNIQUE...")
        
        # Message début
        await self.bot.send_message(
            chat_id=self.chat_id,
            text="🚀 **QUANTUM SCANNER 210K - SCAN DÉMARRÉ**\nRecherche de pépites < 210k€...",
            parse_mode='Markdown'
        )
        
        # Scan des launchpads
        nouveaux = await self.scanner_launchpads()
        print(f"🔍 {len(nouveaux)} projets trouvés")
        
        projets_analyses = 0
        projets_go = 0
        
        for projet in nouveaux:
            # Données projet réalistes < 210K
            projet_data = {
                'nom': projet['nom'],
                'symbol': 'NEW', 
                'mc': random.randint(50000, 200000),  # 🚨 TOUJOURS < 210K
                'website': 'https://vrai-site.com',
                'twitter': 'https://twitter.com/vrai_compte',
                'telegram': 'https://t.me/vrai_channel',
                'fdmc': 250000,
                'circ_supply': 0.3,
                'total_supply': 1.0,
                'volume_24h': 5000,
            ...
            if resultat and resultat['go_decision']:
                projets_go += 1
                print(f"✅ PROJET GO: {resultat['nom']} - MC: {resultat['mc']}€")
...
💰 **FILTRE: MARKET CAP < 210K€**

💎 **{projets_go} pépites détectées sous 210k€**
...
# 🚨 NOUVEAU CODE POUR GITHUB ACTIONS
async def main():
    parser = argparse.ArgumentParser(description='Quantum Scanner 210K')
    parser.add_argument('--once', action='store_true', help='Run single scan')  # 🚨 ARGUMENT --once AJOUTÉ
    args = parser.parse_args()
    
    scanner = QuantumScannerUltime()
    
    if args.once:
        await scanner.run_scan_once()  # 🚨 SCAN UNIQUE
    else:
        await scanner.run_scan_24_7()  # MODE 24/7 ORIGINAL
